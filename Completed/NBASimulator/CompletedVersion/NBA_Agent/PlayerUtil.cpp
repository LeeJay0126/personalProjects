...
void display(T * n);
template<typename T>
void displayStats(T * n, string header[]);
template<typename T>
void displayAwards(T * n, string header[]);
template<typename T>
...
void assignTeams(T * n){
    ...
    while (option != 3){
        ...
        cin >> option;
        if(option == 1){
            cout << "whick player would you like to add to the trade (Enter the number next to their names) : ";
            cin >> playerSelection;
            if (teamSelection == 1){
    ...
    //cout << option << " why are you not 0" << endl;
    cout << "which team would you like to trade with" << endl;
    int otherTeamSelection = 0;
    if (teamSelection == 1){
        cout <<"1.lakers \n2.nets \n3.warriors" << endl;
    ...
    while (option != 3){
        ...
        if(option == 1){
            ...
            //cout << otherTeamValue << " dont be really big please " << endl;
        }
        if(option == 2){
            int year = 2021;
            count = 0;
            ...
            int tradeProbability = value - otherTeamValue;
                cout << value << "   " << otherTeamValue << endl;
            if(tradeProbability > -20 && tradeProbability <= 30){
                cout << "the trade will most likely work" << endl;
            }else if(tradeProbability <= -20 && tradeProbability >= -50 ){
            ...
            }else if(tradeProbability > 30){
                cout << "this trade will work" << endl;
            }
...
void displayAwards(T * n, string header[]){
    ...
    cout <<left << setw(25) << header[0] << setw(15) << header[7]<< setw(15) << header[8]<< setw(15) << header[9]<< setw(20) << header[10]<< setw(20) << header[11]<< setw(20) << header[12]<< setw(10) << header[13]<< setw(10) << header[14]<< setw(10) << header[15]<< setw(10) << header[16]<< setw(10) << header[17]<< setw(15) << header[18]<< setw(10) << header[19] << setw(10) << header[20] <<endl;
    ...
    while(n != nullptr){
        ...
        cout << left << fixed <<setprecision(2) << setw(25) << p.getElement().getName() << setw(15) << p.getElement().getAllNba() << setw(15) << p.getElement().getAllNbaSecond() << setw(15) <<
        p.getElement().getAllNbaThird() << setw(20) << p.getElement().getAllNbaDefense() << setw(20) << p.getElement().getAllNbaDefenseSecond() << setw(20) << p.getElement().getAllNbaDefenseThird() << setw(10) << p.getElement().getAllStarMVP() << setw(10) << p.getElement().getROTY() << setw(10) << p.getElement().getDPOY() << setw(10) << p.getElement().getSMOTY() << setw(10) << p.getElement().getTMOTY() << setw(18) <<
...
template<typename T>
void displayPlayerRank(T * n){
    
    T p;
    int playerSize = 60;
    int playerRank[playerSize];
    string playerName[playerSize];
    int count = 0;
    
    while(n != nullptr){
            
        p = n->getElement();
        playerRank[count] = p.getElement().getPlayerWorth();
        playerName[count] = p.getElement().getName();
        count++;
    ...
    for(int i = 0; i < playerSize; i++){
        for(int j = 0; j < playerSize -1; j++){
            if(playerRank[j]<playerRank[j+1]){
                temp = playerRank[j];
                temp1 = playerName[j];
                playerRank[j] = playerRank[j+1];
                playerName[j] = playerName[j+1];
                playerName[j+1] = temp1;
                playerRank[j+1] = temp;
            }    
        }
    }
    ...
    cout << left << setw(40) << "Name" << setw(10) << "Player Worth" << setw(10) <<  "Rank" << endl;
    
    for(int k = 0; k < playerSize; k++){
        
        cout << left << setw(40) << playerName[k] << setw(10) << playerRank[k] << setw(10) << k+1 << endl;
...
void displayStats(T * n, string header[]){
    ...
    cout <<left << setw(25) << header[0] << setw(5) << header[1] << setw(10) << header[2]<< setw(10) << header[3]<< setw(10) << header[4]<< setw(10) << header[5]<< setw(10) << header[6]<<endl;
    ...
    while(n != nullptr){
        ...
        cout << left << fixed <<setprecision(2) << setw(25) << p.getElement().getName() << setw(5) << p.getElement().getFg() << setw(10) << p.getElement().getRebounds() << setw(10) << p.getElement().getAssists() << setw(10) <<  p.getElement().getSteals() << setw(10) << p.getElement().getBlocks() << setw(10) << p.getElement().getPoints() << endl;
...
void display(T * n, string header[]){
    ...
    cout <<left << setw(25) << header[0] << "Player Worth" << endl;
    ...
    while(n != nullptr){
        ...
        cout << left << fixed <<setprecision(2) << setw(25) << p.getElement().getName() << p.getElement().getPlayerWorth()<<endl;
...
void readFile(T** head_ref,string filename, string header[]){
    ...
    while(getline(input,line)){
        if(count == 0){
            ...
            player.setPlayerCareer(stoi(line));
            ...
            if(player.getPlayerCareer() <= 5 && sum >= 50){
                sum = sum*2;
            }
            
            if(player.getPlayerCareer() <15){
                sum = sum + (player.getPlayerCareer() * 15);
            }else{
                sum = sum + (player.getPlayerCareer() * 12);
            }
